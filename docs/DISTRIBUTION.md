...
This document defines **how the lhtask plugin is distributed** and **how the public
...
claude plugin marketplace update lhtask-marketplace
claude plugin update lhtask
```
...
- **The vendored chain is self-contained.** After bootstrap, the consumer's
  `scripts/lhtask-*.sh`, `.githooks/`, `.claude/agents/` and `lhtask.conf` run with zero
  ...
  the installed chain.
- **Updates are pull-based.** `/lhtask:update` is run *inside the consumer repo, by its
...
3. Consumers: `claude plugin marketplace update lhtask-marketplace && claude plugin
   update lhtask`, then `/lhtask:update` inside each repo when convenient.
