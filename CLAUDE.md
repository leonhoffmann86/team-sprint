...
- three **skills** (`skills/ticket`, `skills/bootstrap`, `skills/update`) — markdown prompt files
  with frontmatter; the skills register the namespaced `/sprint:*` slash commands themselves —
...
- `docs/DISTRIBUTION.md` — the **binding distribution & separation model**: GitHub is the only
  ...
  flows one-way plugin → consumer; updates are pull-based (`/sprint:update` run *inside* the
  consumer repo); the registry (`~/.config/sprint/registry`) is opt-in — `bootstrap` asks before
...
edit `agents/` and run `make sync-agents` to copy them over, then `/sprint:update` in target repos.
...
- commit changed `TODO.md` → **`sprint-plan.sh`** (writes `TODO.autoplan.md`) → chains
  **`sprint-implement.sh`** in the same detached run. The plan stage exits 0 *without* a claude
  run when no active checkbox item remains after `sprint_strip_skipped` — the guard is
...
- commit changed any `SPRINT_REVIEW_DIRS/` → **`sprint-review.sh`** (writes `TODO.review.md`,
  report-only; for single-commit targets it also runs fallow and appends a `### Fallow` section,
  report at `.git/sprint-fallow.json`; every report ends with a `### Tooling` section).

`sprint-implement.sh` is a **shell-driven subagent-team orchestrator** in an **isolated
`git worktree`** on `SPRINT_IMPL_BRANCH` (default `autoplan/impl`). The worktree is a sibling
directory *outside* the repo (`../.sprint-worktree-<repo>`) — never under `.git/`, because the
...
once, then a bounded loop (up to `SPRINT_MAX_ITER`, default 3):

1. **implementer** — smallest change, **one commit per item** (code + `TODO.md`→`DONE.md` +
   `AGENT_LOG.md`),
2. **deterministic gate** (`sprint-gate.sh`, pure shell, no LLM) — lint/typecheck/test/build per
   `SPRINT_GATE_*`/`SPRINT_STACK` (stack auto-detected from marker files), plus a fifth **fallow**
   ...
   `.sprint-state/fallow.json` for the loopback prompt and the reviewers); red → loop back with the
...
`--append-system-prompt` (frontmatter stripped by `sprint_agent_body`); roles exchange JSON sidecars
in `.sprint-state/` inside the worktree (excluded from commits via the worktree's `info/exclude`).
...
`TODO.run.log` via `--output-format stream-json` + the jq renderer `sprint_stream_trace`
(`⚙ implementer → Edit: app/x.py` … `✔ implementer done — 7 turns`), so `tail -f TODO.run.log`
is a real-time status view instead of staying silent for a whole phase. `SPRINT_STREAM=auto`
(default) is jq-gated — `off` or no jq = exactly the pre-0.9.0 silent-until-done behavior — and
non-JSON lines (real stderr errors) pass through verbatim. `sprint_stream_setup` computes the
flags once per stage; the role label reaches the pipeline filter via the shell variable
`SPRINT_TRACE_ROLE`, not the env prefix of the claude process.

On convergence or exhaustion, `sprint_findings_surface` publishes `TODO.review.md` (sections:
Gate · Fallow · Model fallbacks · Reviews · Delivery (only when `SPRINT_DELIVERY=apply`) ·
Tooling) and the
`## 🔎` pointer — the in-loop reviewers replace the old terminal `sprint-review.sh` call (the hook
can't review agent commits because they set `AUTOPLAN_AGENT=1`; `SPRINT_REVIEW_AUTONOMOUS=0` leaves
a gate-only loop). The traffic-light summary (`sprint_surface_review`) counts only **line-leading**
...
`templates/scripts/sprint-lib.sh` holds shared helpers sourced by all stages, including the gate.
...
- **Skip convention:** `sprint_strip_skipped` (in `sprint-lib.sh`) removes items inside `<!-- … -->`,
...
- **Locking:** each stage takes a `mkdir` lock under `.git/sprint-*.lock`; `sprint_reap_stale_lock`
...
- **Detached by default:** stages background themselves so the commit returns immediately. Set
  `SPRINT_FOREGROUND=1` to run synchronously (this is the debugging/testing lever).
...
- **Tooling visibility (graceful, never silent):** degraded tooling must be REPORTED —
  `sprint_tooling_to_md` writes the `### Tooling` section of every `TODO.review.md` (in-loop via
  `sprint_findings_surface` and standalone in `sprint-review.sh`): codegraph (binary **and** repo
  ...
  proxy probe) and the desktop notifier only when `SPRINT_NOTIFY=1`. A deliberate `off`
  config shows as a neutral note, ⚠️ counts into the traffic-light summary. Gate checks skipped
  because their **tool is missing** render as ⚠️ with an install/`SPRINT_GATE_<NAME>` hint
  (fallow: `SPRINT_FALLOW_CMD`) via `sprint_json_checks_to_md` — this covers all per-stack tools
...
- **Permission hardening:** `AUTOPLAN_AGENT=1` is set centrally in `run_phase` (never per
  call-site). Every role gets the hard deny rules from `sprint_deny_settings` via `--settings`
...
- **Fail-closed review parsing:** `sprint_review_max_severity` treats a missing, empty, or
  unparseable review sidecar as `blocker`. Keep that direction — a garbled report must loop back,
  not pass. (Exception by design: `sprint_fallow_to_md` is fail-OPEN — a missing `fallow.json`
  just means fallow didn't run; the gate already enforced the verdict where it matters.)
- **Delivery never auto-commits:** with `SPRINT_DELIVERY=apply` (default stays `branch`), FULLY
  converged work (gate green + reviews ok) is staged into the user's working tree via
  `git merge --squash` (`sprint_apply_impl` in `sprint-lib.sh`) — IDE-native review, the *user*
...
- **Graceful but LOUD model fallback:** a configured cross-vendor model that does not run falls
  back to the Claude chain AND is recorded via `sprint_model_fallback_note` — surfaced as ❌ under
  ...
  missing/unparseable — that one gets ONE forced-Claude retry (`SPRINT_FORCE_CLAUDE=1`, unset
  right after) before fail-closed applies. Degradation is acceptable; *silent* degradation is not.

## Configuration is the single source of truth

`templates/sprint.conf` defines every tunable (review dirs, test command with `{path}` placeholder,
constitution files, impl branch, delivery mode (`SPRINT_DELIVERY` — `branch` default, `apply` =
squash-stage converged work into the working tree), venv to symlink, codegraph mode, the model block — global
`SPRINT_MODEL` plus the per-role overrides `SPRINT_MODEL_{PLAN,PLANNER,NAVIGATOR,IMPLEMENTER,
REVIEWER_CORRECTNESS,REVIEWER_CONVENTIONS,REVIEW}`, resolved per phase by `sprint_model_flags [role]`
(role-specific → `SPRINT_MODEL` → CLI default; role names map uppercase with `-`→`_`, e.g.
`reviewer-correctness` → `SPRINT_MODEL_REVIEWER_CORRECTNESS`) so implementer and reviewers can run
on different models. A per-role value of the form `openrouter:<vendor>/<model>` runs that role
**cross-vendor** behind the Anthropic-compatible translating proxy `SPRINT_PROXY_URL` (e.g. LiteLLM
`/v1/messages`; setup: `docs/CROSS-VENDOR.md`) — `sprint_model_flags` injects
`ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` per role *process* only, sibling roles stay on the
native API. `SPRINT_PROXY_TOKEN` and other machine-local secrets belong in `~/.config/sprint/env`
(never the committed conf), which `sprint_load_config` sources *after* the repo's `sprint.conf` so
it wins. The conf further holds the autonomous-review
and notify toggles, plus the subagent/gate block: `SPRINT_STACK`, the four `SPRINT_GATE_*` commands,
the fallow keys `SPRINT_FALLOW` (`auto`/`off`) and `SPRINT_FALLOW_CMD` (full command override,
`{base}` placeholder), `SPRINT_MAX_ITER`, `SPRINT_PHASE_TIMEOUT`, `SPRINT_STREAM` (live
tool-call trace in `TODO.run.log`: `auto` jq-gated / `off`), and the stage-2 visual-reviewer keys
`SPRINT_VISUAL_MAX_DIFF_RATIO`/`SPRINT_DEV_URL`). Defaults are duplicated in two places that
**must stay in sync** with the conf: `sprint_load_config` in `sprint-lib.sh`, and the inline
defaults at the top of `post-commit` (the hook reads only `SPRINT_REVIEW_DIRS` and
`SPRINT_CODEGRAPH` before scripts source the full lib). Empty `SPRINT_GATE_*` keys fall back to
built-in per-stack defaults in `sprint_gate_cmd` (test additionally falls back to the legacy
`SPRINT_TEST_CMD`).

## The constitution preamble

`sprint_preamble` (in `sprint-lib.sh`) is prepended to every agent prompt and forces the agent to
read the project's constitution files (`SPRINT_CONSTITUTION_FILES`, default `AGENTS.md`) first and
...
Skills are markdown with YAML frontmatter (`name`, `description`, `argument-hint`). `ticket` is a
...
files — scripts, hooks, agents; never `sprint.conf` or lifecycle files; `--all` consumes the registry
at `~/.config/sprint/registry` — registering is exclusively `bootstrap`'s job, opt-in with an
...
(`~/.claude/plugins/cache/*/sprint/*/templates`) when `CLAUDE_PLUGIN_ROOT` is unset — and otherwise
...
so keep it specific and outcome-oriented. The skills register the `/sprint:*` slash commands
...
`SPRINT_MODEL*` keys in `sprint.conf`, never from agent frontmatter. `reviewer-visual` is a
...
loop-guarded by `SPRINT_DOCS_PUSH_GUARD` and gated so headless `claude` only runs on real source
changes. Levers: `SPRINT_DOCS_SKIP=1 git push`, kill switch `touch .git/docs-refresh.disabled`.
...
covering the `sprint_model_flags` resolution chain (role beats global, fallback, name mapping) and
its cross-vendor branch (prefix parsing, env injection, no-proxy/unreachable fallback + recording,
forced-Claude retry, `sprint_model_is_xvendor`) plus the tooling surface (`sprint_tooling_to_md`
reports every supporting tool; `off` → neutral note; conditional curl/notifier lines; missing-tool
gate skips rendered as ⚠️ with config hint), the delivery helper (`sprint_apply_impl`: happy path
...
(`sprint_surface_review`: only line-leading ❌ raises the 🔎 pointer, prose mentions don't)
and the live stream trace (`sprint_stream_setup`/`sprint_stream_trace`: NDJSON events → terse
activity lines, non-JSON noise kept verbatim, `off`/no-jq = pure passthrough),
then bootstraps the plugin into a throwaway repo
(`claude -p --plugin-dir … "/sprint:bootstrap"`), commits a `TODO.md` task, runs the chain with
`SPRINT_FOREGROUND=1`, and asserts `TODO.run.log` was produced. The E2E part needs the `claude`
CLI, so it is not run in CI. To debug a change manually, bootstrap into a throwaway git repo and use:

```bash
SPRINT_FOREGROUND=1 .githooks/post-commit   # run the triggered stage synchronously
tail -f TODO.run.log                        # live activity trace — every agent tool call (reset each trigger)
cat .git/sprint-implement.log               # raw per-stage log
